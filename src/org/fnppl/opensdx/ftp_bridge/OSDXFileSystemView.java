...
import org.fnppl.opensdx.file_transfer.NonExistingRemoteFile;
...
public class OSDXFileSystemView implements FileSystemView {
	
	private OSDXFileTransferClient transfer = null;
	private OSDXFile root = null;
	...
	public static void main(String[] args) {
		...
		try {
			...
			OSDXFileSystemView fs = new OSDXFileSystemView(c);
			
			fs.changeWorkingDirectory("/");
			FtpFile file = fs.getFile("/");
			System.out.println("dir: "+file.getAbsolutePath()+"/"+file.getName());
			List<FtpFile> list = file.listFiles();
			
			for (FtpFile f : list) {
				System.out.println("list :: "+f.getAbsolutePath()+"/"+f.getName());
			}
			
			fs.changeWorkingDirectory("/");
			FtpFile dir = fs.getFile("/test-dir/bla");
