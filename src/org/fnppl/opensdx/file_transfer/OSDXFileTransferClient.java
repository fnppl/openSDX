				...
				private void upload(int no) {
					...
					if (baseDirectory==null) {
						...
						try {
							...
							filenameTo += from.getCanonicalPath().substring(baseDirectory.length());
						} catch (IOException e) {
							filenameTo += from.getName();
							e.printStackTrace();
						}
					}
					System.out.println("upload file "+(no+1)+" of "+count+" :: "+from.getAbsolutePath());
