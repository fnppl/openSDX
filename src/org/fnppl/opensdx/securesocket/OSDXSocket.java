...
public class OSDXSocket implements OSDXSocketSender, OSDXSocketDataHandler {
		...
		if (secureConnectionEstablished) {
			...
			if (agreedEncryptionKey==null) {
				...
				try {
					...
					if (content.getName().equals("hello_client")) {
						...
						Element eInit = new Element("init_connection");
						Element eLogin = new Element("login");
						eLogin.addContent("username", username);
						Element eEncryption = new Element("session_encryption_key");
						eEncryption.addContent("init_vector", SecurityHelper.HexDecoder.encode(agreedEncryptionKey.getInitVector(), '\0', -1));
						eEncryption.addContent("key_bytes", SecurityHelper.HexDecoder.encode(agreedEncryptionKey.getKeyBytes(), '\0', -1));
						receiver.setEncryptionKey(agreedEncryptionKey);
						
						eInit.addContent(eLogin);
						eInit.addContent(eEncryption);
						
						//pack in encrypted OSDXMessage and send to server
						OSDXMessage session_enc_msg = OSDXMessage.buildEncryptedMessage(eInit, serversEncryptionKey, mySigningKey);
			...
			else if (agreedEncryptionKey!=null) {
				if (text.equals("Secure Connection Established!")) {
	...
	public boolean connect(OSDXKey mySigningKey, String username) throws Exception {
...
//	}
	...
	public static void main(String[] args) {
		OSDXSocket s = new OSDXSocket("localhost", 4221,"/");
		try {
